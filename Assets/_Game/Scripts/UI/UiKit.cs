...
namespace Marchio
{
    public static class UiKit
    {
        ...
        public static TrophyBarView TrophyBar(VisualElement parent)
        {
            var view = new TrophyBarView();
            view.Root = new VisualElement();
            view.Root.AddToClassList("trophy");
            var header = new VisualElement();
            header.AddToClassList("trophy-header");
            view.Next = Label(header, "", "trophy-next");
            view.Remain = Label(header, "", "trophy-remain");
            var bar = new VisualElement();
            bar.AddToClassList("bar");
            bar.AddToClassList("trophy-bar");
            view.Fill = new VisualElement();
            view.Fill.AddToClassList("bar-fill");
            view.Fill.AddToClassList("trophy-fill");
            bar.Add(view.Fill);
            view.Root.Add(header);
            view.Root.Add(bar);
    ...
    public sealed class TrophyBarView
    {
        ...
        public void Set(TrophyRoad trophy, float total)
        {
            Fill.style.width = Length.Percent(trophy.ProgressToNext(total) * 100f);
            if (trophy.HasNext)
            {
                Next.text = "NEXT: " + trophy.Next.title.ToUpperInvariant();
                Remain.text = Mathf.Max(0f, trophy.Next.threshold - total).ToString("0") + " TO GO";
            }
            else
            {
                Next.text = "TROPHY ROAD COMPLETE";
                Remain.text = total.ToString("0");
            }
        }

        public void Animate(TrophyRoad trophy, float from, float to, int ms)
        {
            float start = Time.unscaledTime;
            Set(trophy, from);
            ...
            {
                float t = Mathf.Clamp01((Time.unscaledTime - start) * 1000f / Mathf.Max(1, ms));
                Set(trophy, Mathf.Lerp(from, to, t));
            }).Every(16).Until(() => Time.unscaledTime - start >= ms / 1000f + 0.02f);
        }
    }
}
